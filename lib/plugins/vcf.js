 ...
 * 
 * Working with our sample VCF, command used:
...
function createObservationRecord(studyId, block) {
  var request = {};
  request["params"] = {
    "study" : "id;" + studyId, 
    "role" : "observations", 
...
function createVariantRecordRequest(studyId, block) {
  var params = {};

  var HGVSc = block["VARIANT"]["HGVSc"].replace(/^ENS\w[0-9\.]+:/, "");
  var HGVSp = block["VARIANT"]["HGVSp"].replace(/^ENS\w[0-9\.]+:/, "");
  var pos = parseInt(block["POS"]);

  params["variantType"] = "mutation";
  params["consequence"] = block["VARIANT"]["Consequence"];
  params["type"] = "variant";
  params["gene"] = block["VARIANT"]["HGNC"];
  params["geneId"] = block["VARIANT"]["Gene"];

  // There might not be a protein-level variant; if so, use a DNA-level name
  params["mutation"] = HGVSp || HGVSc;

  params["transcript"] = block["VARIANT"]["Feature"];
  params["chromosome"] = block["#CHROM"].replace(/^chr/, "");
  params["cdsPosition"] = block["VARIANT"]["CDS_position"];
  params["codon"] = block["VARIANT"]["Protein_position"];
  params["start"] = pos + 1;
  params["stop"] = pos + block["REF"].length;
  params["HGVSc"] = HGVSc;

  params["originStudyId"] = studyId;
  params["originSample"] = block["SAMPLE"];

  return {body: { data: params}};
}

function recordObservations(db, data, req, res, callback) {
  ...
  tracker.findEntity(db, studyId, "samples", identity, function(err, sample) {
    ...
    function recordObservationList(db, blocks, req, res, callback) {
      if (blocks.length == 0) {
      ...
      if (block["VARIANT"]["Consequence"] === "synonymous_variant") {
        return recordObservationList(db, blocks, req, res, callback);
      }

      var request = createObservationRecord(studyId, block);
      request["body"]["data"]["step"]["fields"]["sampleEntityRef"] = {"ref" : new BSON.ObjectID(sampleIdentifier.toString())};
      request["body"]["data"]["step"]["fields"]["participantEntityRef"] = {"ref" : new BSON.ObjectID(participantIdentifier.toString())};
      request["user"] = req["user"];

      // console.log("About to write entity step", sys.inspect(request, false, 6));
      tracker.postEntityStep(err, db, request, res, function(db, err, entityUrl, statusCode) {
        // console.log("Maybe all is well", err, url, statusCode);
        if (! err) {
          var request = createVariantRecordRequest(studyId, block);
          request["body"]["apikey"] = res.locals.config["apikey"];
          var endpoint = res.locals.config["baseUrl"] + res.locals.config["knowledgeUriBase"] + "/variants";
          ...
          var httpRequest = http.request(options, function(httpResponse) {
            ...
            httpResponse.on('end', function (message) {
              return recordObservationList(db, blocks, req, res, callback);
            });
          });
...
function parseVcfFile(db, data, req, res, callback) {
  ...
  gridStore.open(function(err, gs) {
    gridStore.readlines(function(ignore, lines) { 
      parseVcfLines(db, data, lines, req, res, function() {
        recordObservations(db, data, req, res, callback);
      });
    })
  });
}
  
function parseVcfLines(db, data, lines, req, res, callback) {
...
function handleVcf(db, data, req, res, callback) {
  //console.log("Calling parseVcfFile", sys.inspect(data, null, 2));

  // This worries me: why is the error flag totally ignored here? 
  parseVcfFile(db, data, req, res, function(err, response) {
