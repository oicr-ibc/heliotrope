...
function parseVcfFile(db, data, callback) {
  ...
  gridStore.open(function(err, gs) {
    ...
    parseVcfStream(db, data, stream, function() {
      callback(null, data);
    })
  });
}
  
function parseVcfStream(db, data, stream, callback) {
  ...
  function handleDataRow(data) { 
    ...
    //console.log("Block", sys.inspect(block, null, 2));
    blocks.push(block);
  }
  
  var lazyLines = new lazy(stream);
  
  /**
   * Iterates lazily through the file's input stream. As close to a
   * common file-based iteration as is easily possible. 
   */
  lazyLines.on('end', function() {
    
    // At the end, let's take a peek at what we have. 
    console.log("Data", sys.inspect(data, false, 2));
    console.log("Block", sys.inspect(blocks, false, 2));
...
function handleVcf(db, data, callback) {
  ...
  parseVcfFile(db, data, function(err, response) {
    console.log("Called handleVcf", sys.inspect(response, null, 2));
