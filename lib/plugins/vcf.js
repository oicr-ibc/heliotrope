...
function recordObservations(db, data, response, callback) {
  ...
  tracker.findEntity(db, studyId, "samples", identity, function(err, sample) {
    ...
    function recordObservationList(db, blocks, response, callback) {
      if (blocks.length == 0) {
      ...
      if (block["VARIANT"]["Consequence"] === "synonymous_variant") {
        return recordObservationList(db, blocks, response, callback);
      }
      ...
      request["body"]["data"]["step"]["fields"] = {
        ...
        "sampleEntityRef" : {"ref" : new BSON.ObjectID(sampleIdentifier.toString())},
        "participantEntityRef" : {"ref" : new BSON.ObjectID(participantIdentifier.toString())},
      ...
      tracker.postEntityStep(err, db, request, response, function(db, err, url, statusCode) {
...
function parseVcfFile(db, data, response, callback) {
  ...
  gridStore.open(function(err, gs) {
    gridStore.readlines(function(ignore, lines) { 
      parseVcfLines(db, data, lines, response, function() {
        recordObservations(db, data, response, callback);
      });
    })
  });
}
  
function parseVcfLines(db, data, lines, response, callback) {
...
function handleVcf(db, data, res, callback) {
  //console.log("Calling parseVcfFile", sys.inspect(data, null, 2));

  parseVcfFile(db, data, res, function(err, response) {
