...
function transformToXml(doc) {
  ...
  function transform(element) {
    console.log("Transforming", element.toString());
    if (Array.isArray(element)) {
      ...
      xw.endElement();
    } else if (typeof element === "object" && ! element.hasOwnProperty('_bsontype')) {
...
function sendViewResponse(req, res, view) {
  return function(db, err, doc, statusCode) {
    ...
    if(err) {
      ...
      console.log(sys.inspect(doc, false, null));
      res.render(view, doc);
    }
    db.close();
  };
...
app.get('/knowledge/api/variants/:id/report', function(req, res) {
  knowledge.getVariant(req, sendViewResponse(req, res, 'index'));
});
