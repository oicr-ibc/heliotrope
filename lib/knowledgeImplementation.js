...
function getPublication(err, db, req, res, callback) {
  var type = req.params['type'];
  var id = req.params['id'];
  var publicationId = type + ":" + id;
  db.collection("publications", function(err, publications) {

    if (err) { return callback(db, err, {}, res); }

    publications.find({name: publicationId}).limit(1).toArray(function(err, docs) {

      if (err) { return callback(db, err, {}, res); }
      if (docs.length < 1) {
        return callback(db, {err: "no such object: " + publicationId}, "", res, 404);
      }
      var doc = docs[0];
