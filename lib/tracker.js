  ...
  db.collection("steps", function(err, steps) {
    steps.find(stepSelector).toArray(function(err, stepsArray) {
      ...
      Object.keys(fields).forEach(function(fieldKey) {
        ...
        if (stepValues[fieldKey] === undefined && step.fields[fieldKey].hasOwnProperty("default")) {
      ...
      studies.find({name: studyName}, {_id: 1, name: 1}).limit(1).toArray(function(err, docs) {
        ...
        db.collection("steps", function(err, steps) {
          steps.find(stepSelector).toArray(function(err, stepsArray) {
            ...
            try {
              var existingEntityStepUpdater = buildStepUpdater(step, req.body);
            } catch (exception) {
              if (exception instanceof StepError) {
                return callback(db, {data: exception.data, err: exception.message}, "");
            ...
            db.collection("entities", function(err, entities) {
              ...
              entities.update(existingEntityStepSelector, existingEntityStepUpdater, updateOptions, function (err, result) {
                ...
                if (result == 0) {
                  ...
                  if (existingEntityStepSelector["identity"] == "id;new") {
                    existingEntityStepSelector["identity"] = existingEntityStepUpdater["$set"]["identity"];
                  ...
                  entities.update(existingEntityStepSelector, existingEntityStepUpdater, updateOptions, function (err, result) {
                    //console.error("Update step 2 result", JSON.stringify({err: err, result: result}, null, 2))
