...
function findStudy(db, req, callback) {
  ...
  db.collection("studies", function(err, studies) {
    ...
    studies.find(selector, {_id: 1, name: 1}).limit(1).toArray(function(err, docs) {
      if (err) {
...
function getEntity(err, db, req, callback) {
  ...
  findStudy(db, req, function(err, doc) {
    if (err) { callback(db, {err: err}, null); }
    var studyId = new BSON.ObjectID(doc._id.toString());
    ...
    db.collection("steps", function(err, steps) {
      steps.find(stepSelector).toArray(function(err, stepsArray) {
        ...
        findStepUpdater(db, studyId, step, req.body.data.step.fields, {}, initialUpdater, function(db, err, updater) {
          ...
          if (err) {
            console.log("Error from findStepUpdater", err);
          ...
          db.collection("entities", function(err, entities) {
            entities.update(existingEntityStepSelector, updater, updateOptions, function (err, result) {
              ...
              if (err) {
                console.log("Error from entities update existingEntityStepSelector #1", err, existingEntityStepSelector, updater, updateOptions);
              ...
              if (result == 0) {
                ...
                entities.update(existingEntityStepSelector, updater, updateOptions, function (err, result) {
                  ...
                  if (err) {
                    console.log("Error from entities update existingEntityStepSelector #2", err, existingEntityStepSelector, updater, updateOptions);
                    return callback(db, err, newUrl);
                  }
