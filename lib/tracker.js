 ...
 */
function getStudy(err, db, req, callback) {
    ...
    studies.find({name: name}).limit(1).toArray(function(err, docs) {
      ...
      if (req["query"] !== undefined && req["query"].hasOwnProperty('q')) {
        ...
        if (queryRecord === undefined) {
          return callback(db, "Can't find query: " + name, {"error" : "Can't find query: " + name});
        }
 ...
 * @param callback
 */
function getEntity(err, db, req, callback) {
  ...
  db.collection("studies", function(err, studies) {
    studies.find({name: studyName}, {_id: 1, name: 1}).limit(1).toArray(function(err, docs) {
      ...
      // A new entity request at the entity is invalid. This should always be a step which
      // can act as a constructor. As such, we should handle this as a forbidden access
      // with not found. 
      ...
      if (identity.substr(0, 3) === "id;") {
        ...
        if (key == "new") {
          return callback(db, {err: "no such object: " + identity}, "", 404);
        }
              ...
              entities.find(relatedSelector).toArray(function(err, related) {
                ...
                try {
                  ...
                  if (exception instanceof Error) {
                    return callback(db, {err: exception.message}, "");
                  }
                }
              });
            });
 ...
 * @param callback - function accepting (err, doc, db)
 */
function getViews(err, db, req, callback) {
  var name = req.params.study;
  var role = req.params.role;
...
function getEntityStep(err, db, req, callback) {
  var studyName = req.params.study;
  var role = req.params.role;
  var identity = req.params.identity;
...
function getEntities(err, db, req, callback) {
  var studyName = req.params.study;
  var role = req.params.role;
...
function findStepUpdater(db, step, fields, errors, updater, callback) {    
  ...
  if (! step.fields.hasOwnProperty(fieldName)) {
    return findStepUpdater(db, step, fields, errors, updater, callback);
  }
  ...
  if (fieldDefinition.identity) {
    ...
    var identity = fieldValue.value;
    var role = fieldDefinition.entity;
    db.collection("entities", function(err, entities) {
      entities.find({identity: identity, role: role}, {_id: 1}).limit(1).toArray(function(err, docs) {
        if (err) {
          return callback(db, err, null);
        } else if (docs.length == 0) {
          errors.missingObject = ["role", identity];
          return findStepUpdater(db, step, fields, errors, updater, callback);
        } else {
          var doc = docs[0];
          var id = new BSON.ObjectID(doc._id.toString());
          newField["ref"] = id;
          updater["$set"]["steps.$.fields"].push(newField);
          return findStepUpdater(db, step, fields, errors, updater, callback);
        }
...
function buildStepUpdater(step, body) {
  var fields = body.data.step.fields || {};
  
  var stepFields = step.fields;
  
  console.error("Building step updater", step, body, fields);
  
  // Are there any fields required which we failed to get values for. And this
  // means a real value, i.e., not null.
  var missingFields = [];
  Object.keys(stepFields).map(function(stepFieldKey) {
    if (! fields[stepFieldKey]) {
      missingFields.push(stepFieldKey);
    }
  });
  if (missingFields.length > 0) {
    var message = "missing fields: " + missingFields.join(", ");
    throw new StepError(message, {missingFields: missingFields});
  }
  
  var newIdentity = undefined;
  var result = Object.keys(fields).map(function(fieldKey) {
    var fieldData = {"key" : fieldKey};
    if (stepFields[fieldKey].identity) {
  ...
  //console.error("Building", newIdentity, result["$set"], body.data.step);
...
function postEntityStep(err, db, req, callback) {
  var studyName = req.params.study;
  var role = req.params.role;
  var identity = req.params.identity;
  var stepName = req.params.step;
    ...
    studies.find({name: studyName}, {_id: 1, name: 1}).limit(1).toArray(function(err, docs) {
      ...
      db.collection("steps", function(err, steps) {
        steps.find(stepSelector).toArray(function(err, stepsArray) {
          ...
          findStepUpdater(db, step, req.body.data.step.fields, {}, initialUpdater, function(db, err, updater) {
            
            if (err) {
              return callback(db, err, err.toString(), 400);
            }
                ...
                if (result == 0) {
                  ...
                  entities.update(existingEntityStepSelector, updater, updateOptions, function (err, result) {
                    ...
                    if (result == 0) {
                      ...
                      return callback(db, "Internal error inserting document", newUrl)
...
function storeEntityFiles(db, data, files, identifiers, callback) {
  ...
  console.log("Writing file", identifier);
 ...
 */
function postEntityStepFiles(err, db, req, callback) {
  ...
  if (err) {
    return callback(db, err, req.url, 500);
  }
    ...
    studies.find({name: studyName}, {_id: 1, name: 1}).limit(1).toArray(function(err, docs) {
      ...
      db.collection("steps", function(err, steps) {
        ...
        steps.find(stepSelector).limit(1).toArray(function(err, stepsArray) {
          ...
          function finalize(err, response) {
            if (step.hasOwnProperty('plugin')) {
              ...
              method(db, response, function(err, data) {
                return callback(db, {"err": null, "status": "OK", "data" : data}, null, 200);
              });
            } else {
              return callback(db, {"err": null, "status": "OK", "data" : response}, null, 200);
            }
          }
          ...
          if (req.form.ended && Object.keys(req.files).length > 0) {
            console.log("Maybe need to end manually, because we ended the form before we got here?", sys.inspect(req.form, null, 2), sys.inspect(req.files, null, 2));
            onEnd();
          }
        });
      });
