...
/**
 * Checks the current request against an access structure, usually attached to
 ...
 * @param mode - a string, w for write access, and r for read access
 */
function checkAccess(req, access, mode) {
  var user = req.user;
  assert.ok(user, "Can't find user in request");

  if (req.user.roles && req.user.roles.indexof('ADMIN') !== -1) return true;

  var userId = user.userId;
  var modifyAllowed = access.modify && access.modify.indexOf(userId) !== -1;
  if (modifyAllowed) return true;

  var readAllowed = access.read && access.read.indexOf(userId) !== -1;
  if (mode !== 'w') return readAllowed;
...
function findStudy(db, req, res, callback) {
  ...
  db.collection("studies", function(err, studies) {
    ...
    studies.find(selector).limit(1).toArray(function(err, docs) {
      ...
      if (! checkAccess(req, doc.access, 'r')) {
...
function getStudy(err, db, req, res, callback) {
  findStudy(db, req, res, function(err, doc) {
