          ...
          db.collection("entities", function(err, entities) {
            entities.find(relatedSelector).toArray(function(err, related) {
              ...
              try {
                ...
                var plugin = doc["roles"] && doc.roles[role] && doc.roles[role].readEntity;
                return maybeInterceptedCallback(plugin, db, err, result, callback);
              }
            });
          });
        });
        ...
        function finalize(err, response) {
          ...
          if (step.hasOwnProperty('plugin')) {
            ...
            method(db, response, function(err, data) {
              return callback(db, null, {data: data}, 200);
            });
          } else {
            return callback(db, null, {data: response}, 200);
          }
        }
