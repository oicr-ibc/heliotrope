...
function buildEntityStepUrls(entity, stepsArray) {
  ...
  entity.steps.forEach(function(step) {
    ...
    if (stepDefinition) {
      ...
      step.url = url + "/step/;" + step.id.toString();
    }
  });
}
  ...
  db.collection("steps", function(err, steps) {
    steps.find(stepSelector).toArray(function(err, stepsArray) {
      ...
      for(var i = 0; i < stepDescriptorCount; i++) {
        var thisStep = stepDescriptors[i];
        if (thisStep.stepRef.equals(step._id)) {
          selectedStep = thisStep;
        }
      }

      // Change the step label to be localized.
      if (selectedStep && step.redoLabel !== undefined) {
        step.label = localize(step.redoLabel);
      } else {
      ...
      // And finally, related entities. 
      var relatedSelector = buildRelatedEntitySelector(entity, step, query);
      db.collection("entities", function(err, entities) {
        entities.find(relatedSelector).toArray(function(err, related) {
          ...
          if (selectedStep) {
            Object.keys(fields).forEach(function(fieldKey) {
              ...
              var fieldCount = selectedStep.fields.length;
              for(var j = 0; j < fieldCount; j++) {
                if (selectedStep.fields[j].key == fieldKey) {
                  currentValueObject = selectedStep.fields[j];
                }
              }
              ...
              if (typeof currentValueObject === "object") {
                Object.keys(currentValueObject).forEach(function(valueKey) {
                  if (valueKey !== "key") {
                    ...
                    step.fields[fieldKey][valueKey] = currentValueObject[valueKey];
                  }
                });
              }
            });
            ...
            Object.keys(fields).forEach(function(fieldKey) {
              ...
              if (queryValue) {
                if (field.isIdentity) {
                  step.fields[fieldKey]["identity"] = queryValue
                } else if (field["type"] === "Reference") {
                  related.forEach(function(relatedEntity) {
                    if (relatedEntity.identity === queryValue) {
                      step.fields[fieldKey]["ref"] = relatedEntity._id;
                      step.fields[fieldKey]["value"] = relatedEntity.identity;
                    }
                  })
                } else {
                  step.fields[fieldKey]["value"] = queryValue;
                }
              }
            });
          ...
          buildFieldsDisplayValues(step.fields, related);
...
function postEntityStep(err, db, req, res, callback) {
  ...
  // console.log("postEntityStep", req.body.data);
    ...
    findEntity(db, studyId, role, identity, function(err, entity) {
      ...
      db.collection("entities", function(err, entities) {
        var cursor = entities.find(selector).toArray(function(err, results) {
          ...
          return callback(db, err, {data: sorted}, res);
        });
      });
    });
  });
