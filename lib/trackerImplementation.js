  ...
  return {
    ...
    callback : function(object, results, next) {
      ...
      results.forEach(function(e) {
        ...
        if (e["description"]) {
          e["description"] = localize(e["description"]);
        }
...
function checkAdminAccess(req, mode) {
  ...
  if (req.user.roles && req.user.roles.indexOf('TRACKER_ADMIN') !== -1) return true;
  return false;
}
...
function buildEntityValues(entity, related, stepsArray) {
  ...
  var availableSteps = stepsArray.map(function(step) {
    ...
    if (step["description"]) {
      stepDefinition["description"] = localize(stepDefinition["description"]);
    }
      ...
      db.collection("steps", function(err, steps) {
        steps.find(stepSelector).toArray(function(err, allStepsArray) {
          ...
          stepsArray = stepsArray.filter(function(step) {
            return checkAdminAccess(req, 'read') || checkAccessList(req, accessList(step, doc), 'read');
          });
              ...
              if (result == 0) {
                ...
                entities.update(existingEntityStepSelector, updater, updateOptions, function (err, result) {
                  ...
                  if (result == 0) {
                    ...
                    return callback(db, "Internal error inserting document", newUrl, res, 400)
...
function getStudyRelatedEntities(err, db, req, res, callback) {
  var studyName = req.params.study;
...
module.exports.getStudyRelatedEntities = getStudyRelatedEntities
