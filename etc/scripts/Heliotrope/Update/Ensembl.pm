...
sub maybe_update {
    ...
    $req = HTTP::Request->new(GET => "ftp://ftp.ensembl.org/pub/release-69/mysql/");
    ...
    my $base_url = "ftp://ftp.ensembl.org/pub/release-69/mysql/${directory}_${version}";
...
__ENDSQL__
    ...
    $self->load_data($registry, "protein_feature.txt.gz",
       qq{INSERT INTO protein_feature (protein_feature_id, translation_id, seq_start, seq_end, hit_start, hit_end, hit_name, analysis_id, score, evalue, perc_ident, external_data) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)},
       <<__ENDSQL__);
